    ...
    return new Promise((resolve, reject) => {
      ...
      req.onload = function () {
        if (req.status === 200) {
          ...
          resolve(req.response); // You'll want to resolve with the data from req.response
        } else {
          console.log("It failed :("); // It failed :(
          reject(req.statusText); // Be nice and reject with req.statusText
        }
      };
      req.onerror = function () {
        console.log("It failed :("); // It failed :(
        reject("Network Error"); // Pass a 'Network Error' to reject
      };
      req.send();
    });
  }
